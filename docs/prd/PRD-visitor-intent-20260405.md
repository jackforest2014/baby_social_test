...
- [8. 里程碑计划](#8-里程碑计划)
- [9. 待确认事项](#9-待确认事项)
- [10. 附录：skill_base.md 优化建议](#10-附录skill_basemd-优化建议)
- [11. 附录：主人接待设置（原 owner_profile.md）能力支撑分析](#11-附录主人接待设置原-owner_profilemd能力支撑分析)
  - [11.1 原始设计与能力模型的映射分析](#111-原始设计与能力模型的映射分析)
  - [11.2 核心问题：文件模式的结构性缺陷](#112-核心问题文件模式的结构性缺陷)
  - [11.3 系统引导式填写的设计方向](#113-系统引导式填写的设计方向)
  - [11.4 对 owner_profile.md 的定位调整](#114-对-owner_profilemd-的定位调整)
  - [11.5 技术方案落地要点](#115-技术方案落地要点)
...
AI 助理的对话行为由两个数据源在运行时合并构成 system prompt：

| 数据源 | 维护者 | 存储方式 | 内容职责 |
| ------ | ------ | -------- | -------- |
| `skill_base.md` | 开发者 | 文件（版本化管理） | 通用对话流程、红线规则、各类访客处理方式、信息收集规范、收尾话术框架 |
| AI 助理接待设置 | 主人 | 数据库（结构化字段） | 主人名字、身份标签、对外简介、FAQ、接待偏好、信息收集偏好、对话风格、语言偏好 |

**主人接待设置的采集方式**：通过系统引导式填写流程（而非用户自行维护文件），分步引导主人完成各项设置。具体引导流程详见 PRD-USER-PROFILE-001 中"AI 助理接待设置"模块。

**系统引导式填写的优势**：
- **安全性**：输入时即可对照红线列表实时校验，拦截用户无意写入的敏感信息（如技术栈、项目名等）
- **完整度**：分步引导 + 字段校验，确保关键字段不遗漏
- **易维护**：表单式交互，无需用户理解 Markdown 或文件格式
- **结构化**：字段直接入库，运行时按需组装为 prompt 片段，减少 token 浪费
- **可追溯**：每次修改自动记录版本，支持回溯和对比

**prompt 注入规则**：接待设置数据由服务端从数据库读取，按结构化字段动态组装为 prompt 片段，以 `<profile>...</profile>` 标签包裹后注入 system prompt。如有冲突，主人明确授权公开的信息可以输出；未明确授权的，一律遵守 `skill_base.md` 的"不知道"原则。
...
本模块核心关注 AI 助理的意图识别准确性和信息提取完整度，这是主人从访客对话中获取价值的关键。

### 7.1 核心指标

| 指标名称 | 定义 | 计算公式 | 目标值 | 统计口径 |
|----------|------|----------|--------|----------|
| 意图识别准确率 | AI 正确识别访客意图类型的比例 | 意图识别正确的会话数 / 有意图标注的会话总数 × 100% | ≥ 85% | 周 |
| 信息提取完整度 | 成功提取到核心结构化信息（来意 + 联系方式）的会话比例 | 提取到≥2个核心字段的会话数 / 对话轮次≥3的会话总数 × 100% | ≥ 70% | 周 |
| 主人会话查阅率 | 主人查看访客会话详情的比例 | 被主人查看的会话数 / 总会话数 × 100% | ≥ 80% | 周 |

### 7.2 过程指标

| 指标名称 | 定义 | 计算公式 | 关注方向 | 统计口径 |
|----------|------|----------|----------|----------|
| 对话完成率 | 访客完整走完对话漏斗（到达收尾阶段）的比例 | 到达步骤5-6的会话数 / 总会话数 × 100% | 反映对话策略是否有效引导 | 周 |
| 平均对话轮次 | 每个会话的平均消息往返次数 | 总消息数 / 总会话数 | 过少可能未有效收集信息，过多可能体验拖沓 | 周 |
| 红线触发率 | 触发信息保护机制（拒绝回答敏感信息）的会话比例 | 触发红线的会话数 / 总会话数 × 100% | 监控攻击频率和防护有效性 | 日 |
| 访客主动离开率 | 访客在步骤3之前停止对话的比例 | 步骤3之前离开的会话数 / 总会话数 × 100% | 过高说明开场或引导策略需优化 | 周 |
| 意图类型分布 | 各意图类型的会话占比 | 各类型会话数 / 总会话数 × 100% | 了解访客构成，优化策略优先级 | 月 |
| 会话未读积压量 | 主人未查看的会话数量 | COUNT(未读会话) | 积压过多需推送提醒 | 日 |

### 7.3 健康指标

| 指标名称 | 定义 | 告警阈值 | 统计口径 |
|----------|------|----------|----------|
| AI 回复平均延迟 | AI 助理生成回复的平均耗时 | > 3s 需优化 | 实时 |
| 意图识别模块调用失败率 | LLM 意图识别调用失败的比例 | > 2% 需告警 | 实时 |
| 信息提取调用失败率 | LLM 信息提取调用失败的比例 | > 2% 需告警 | 实时 |
| 会话超时率 | 因 5 分钟无活跃而自动结束的会话比例 | 仅监控趋势，无固定阈值 | 日 |
| 对话状态机异常率 | 状态流转出现非预期跳转的比例 | > 0.5% 需排查 | 实时 |
...
## 11. 附录：主人接待设置（原 owner_profile.md）能力支撑分析

本章节分析 `docs/requirements/visitor_intent/owner_profile.md` 中定义的原始字段设计对 AI 助理核心能力模型（第 2.3 节）五个层次的支撑情况。基于分析结论，本 PRD 已将 owner_profile 从"用户自行维护文件"调整为"系统引导式填写 + 结构化存储"（见 3.1.1 节），本节详述调整的依据和设计方向。

### 11.1 原始设计与能力模型的映射分析

`owner_profile.md` 原始设计包含 7 个章节，逐一对照五层能力模型：

| 能力层次 | owner_profile.md 章节 | 支撑程度 | 分析 |
|---------|----------------------|----------|------|
| L1 防护 | 文件说明（"只输出明确写出的内容"）| ⚠️ 部分支撑 | 声明了"不说没写的"原则，但**未显式定义哪些信息绝对不可泄露**。防护层依赖 skill_base.md 的红线列表，owner_profile.md 本身没有"即使我写了也不能说"的负面清单。若主人不慎在 FAQ 或简介中写入了敏感信息（如具体技术栈、项目名），当前机制无法拦截。 |
| L2 识别 | 接待偏好（第五节）| ⚠️ 部分支撑 | "优先跟进的来访类型"和"不感兴趣的来访类型"为意图识别后的**处理策略**提供了依据，但**未提供辅助识别的线索**。例如：主人是投资人还是创业者？主人的行业领域是什么？这些信息能帮助 AI 更准确地判断访客意图（如"投资类访客来找一个投资人"与"投资类访客来找一个创业者"的处理逻辑完全不同）。 |
| L3 收集 | 无直接对应 | ❌ 不支撑 | 收集层需要知道**主人关心什么信息**，以便有针对性地引导收集。当前 owner_profile.md 未定义主人希望从访客处获取的信息优先级。skill_base.md 定义了通用收集字段（来意、背景、联系方式），但缺少主人个性化的收集偏好（如"我特别关心对方的投资阶段"或"我想知道合作的具体形式"）。 |
| L4 引导 | 收尾话术偏好（第六节）| ⚠️ 部分支撑 | 仅覆盖了对话漏斗的**最后一步**（收尾）。对话引导的中间步骤（如如何从闲聊引向来意、如何从来意引向背景信息）缺少主人的个性化偏好。例如：有的主人希望 AI 快速高效，有的主人希望 AI 慢慢聊、建立关系。 |
| L5 适配 | 问候语风格偏好（第一节）、语气风格 | ⚠️ 部分支撑 | 定义了问候语和基调（"轻松友好"），但**仅覆盖了单一维度**。适配层需要根据不同访客类型调整风格，当前缺少分场景的语气指引（如面对投资人用什么语气、面对学生用什么语气）。此外，多语言问候语只有中英两种，与能力模型中列出的日/韩/阿拉伯/西/泰/粤语适配需求存在差距。 |

### 11.2 核心问题：文件模式的结构性缺陷

除上述字段覆盖不足外，"用户自行维护 Markdown 文件"这一模式本身存在结构性问题：

| 问题 | 说明 | 影响 |
|------|------|------|
| **安全无法前置拦截** | 自由文本中无法阻止用户写入敏感信息，只能在 prompt 注入时做事后扫描 | L1 防护层存在漏洞 |
| **字段完整度不可控** | 用户可能大量留空，导致 AI 缺少关键上下文 | L2-L5 全部受影响 |
| **格式不可控** | Markdown 格式错误、缩进混乱、编码问题等可能导致解析失败 | 系统稳定性风险 |
| **维护门槛高** | 用户需理解文件结构、字段含义、Markdown 语法 | 填写率低，数据质量差 |
| **无版本追溯** | 用户覆盖文件后无法回滚到上一版本 | 误操作不可恢复 |

**结论**：采用系统引导式填写 + 结构化存储，从根本上解决上述问题。

此外，`owner_profile.md` 原始字段设计也存在以下具体缺陷：

**缺陷一：缺少防护层负面清单**

当前依赖"只说写了的"这一正向原则，但缺少显式的"即使写了也不能说"的负面清单。主人可能在 FAQ 或简介中无意写入敏感信息（如"我用 Go + PostgreSQL 做的"），AI 会按当前逻辑直接输出，违反 skill_base.md 的红线规则。需要一个**信息安全自检机制**或**敏感字段标注**。

**缺陷二：缺少主人身份的结构化描述**

当前"对外身份介绍"只有一个自由文本的简介字段。AI 助理在识别层需要理解主人的身份才能准确判断访客意图的合理性和处理策略。例如：
- 主人是创业者 → 投资人来访是高优场景
- 主人是技术专家 → 媒体采访关注技术方向
- 主人是自由职业者 → 外包类访客可能是目标客户

缺少结构化的身份标签，AI 只能从自由文本中推断，增加了误判风险。

**缺陷三：缺少个性化信息收集偏好**

skill_base.md 定义了通用的收集字段（来意、背景、联系方式），但不同主人对"有价值信息"的定义不同。例如：
- 创业者可能特别关心访客的资源和合作形式
- 自媒体主可能特别关心访客的粉丝量和平台
- 求职者可能特别关心 HR 的公司和岗位方向

当前无法表达这些个性化收集需求。

**缺陷四：缺少对话节奏偏好**

收尾话术只覆盖了对话的最后一步。主人对 AI 整体对话节奏的偏好未被捕获：
- 高效型：尽快收集信息，减少闲聊
- 关系型：多聊几轮，建立好感后再收集
- 防御型：对所有访客保持高度警惕

对话节奏偏好直接影响 L4 引导层的行为。

**缺陷五：缺少多语言和文化适配配置**

问候语只有中英两种，而能力模型中 L5 适配层需要支持 7+ 种语言。虽然语言切换主要由 skill_base.md 控制，但主人可能希望对不同语言的访客有不同的接待策略（如"日语访客优先跟进"或"仅接待中英文访客"）。

**缺陷六：FAQ 结构不利于 LLM 消费**

当前 FAQ 是固定的预设问题列表，存在两个问题：
1. 主人可能遇到的问题远不止预设的 7 个，缺少自由扩展的引导
2. FAQ 问答对的格式未考虑 LLM 的检索效率，当 FAQ 数量增多时，全量注入 prompt 会增加 token 消耗

### 11.3 系统引导式填写的设计方向

以下设计方向供 PRD-USER-PROFILE-001 的"AI 助理接待设置"模块和后续技术��案参考。

**方向一：分步引导流程**

将接待设置拆分为 6 个步骤，每步聚焦一个主题，降低认知负荷：

```
步骤 1/6：基本信息        → 昵称（助手称呼方式）、问候语
步骤 2/6：对外身份        → 角色类型、行业、当前阶段、对外简介
步骤 3/6：接待偏好        → 优先跟进/不感兴趣的来访类型、是否接受合作/采访/招聘
步骤 4/6：信息收集偏好    → 按访客类��定义希望额外收集的信息
步骤 5/6：对话风格        → 整体节奏、防御等级、收尾话术
步骤 6/6：高级设置        → 语言偏好、FAQ 管理
```

每步提供**示例值**和**解释文案**，用户可跳过非必填步骤，后续随时回来补充。

**方向二：输入时安全校验**

在自由文本字段（对外简介、FAQ 回答）的输入环节，实时对照红线关键词列表进行检测：

- 检测到疑似敏感信息时，**即时提示**用户（如"检测到可能包含技术栈信息'Go + PostgreSQL'，AI 助理不会向访客展示此类���息，建议移除"）
- 提示方式为**建议而非阻断**，尊重���户意愿，但确保用���知情
- 即使用户坚持填入，服务端在组装 prompt 时仍会对照红线列表做二次过滤

**方向三：结构化字段定义**

以下字段取代 owner_profile.md 中的自由文本，直接存储在数据库中：

| 字段组 | 字段 | 类型 | 必填 | 对应能力层 |
|--------|------|------|------|-----------|
| 基本信息 | display_name | 文本 | 是 | L5 适配 |
| 基本信息 | greeting_style | 枚举（轻松友好/正式专业/自定义） | 是 | L5 适配 |
| 基本信息 | custom_greeting | 文本 | 否 | L5 适配 |
| 对外身份 | role_type | 枚举（创业者/独立开发者/技术专家/自媒体/自由职业者/企业管理者/学者/其他） | 是 | L2 识别 |
| 对外身份 | industry | 枚举+自定义 | 否 | L2 识别 |
| 对外身份 | current_stage | 枚举（早期探索/产品研发中/已上线运营/成熟业务） | 否 | L2 识别 |
| 对外身份 | public_bio | 长文本 | 是 | L2 识别、L5 适配 |
| 接待偏好 | priority_visitor_types | 多选标签 | 否 | L2 识别 |
| 接待偏好 | rejected_visitor_types | 多选标签 | 否 | L2 识别 |
| 接待偏好 | accept_cooperation | 枚举 | 否 | L2 识别 |
| 接待偏好 | accept_media | 枚举 | 否 | L2 识别 |
| 接待偏好 | accept_recruitment | 枚举 | 否 | L2 识别 |
| 收集偏好 | custom_collect_fields | JSON（按访客类型定义额外收集字段） | 否 | L3 收集 |
| 对话风格 | conversation_pace | 枚举（高效直接/适度友好/慢热关系型） | 是 | L4 引导 |
| 对话风格 | defense_level | 枚举（标准/偏高/偏低） | 是 | L1 防护、L4 引导 |
| 对话风格 | closing_style | 枚举+自定义 | 否 | L4 引导 |
| 语言偏好 | supported_languages | 多选 | 是 | L5 适配 |
| 语言偏好 | fallback_language | 枚举 | 是 | L5 适配 |
| FAQ | faq_items | JSON 数组（question + answer + tags） | 否 | L1-L5 全层 |
...
- 注册后首次进入接待设置时，仅要求填写步骤 1-2（基本信息 + 对外身份），其余步骤标记为"稍后完善"
- 在主人查看访客会话后，根据实际对话情况推荐优化设置（如"您的访客中 30% 是投资类，是否设置投资类访客的额外收集字段？"）
- 接待设置的完善度纳入资料完善度评分

### 11.4 对 owner_profile.md 的定位调整

调整后，`docs/requirements/visitor_intent/owner_profile.md` 的定位从"用户维护的配置文件"变为：

1. **需求参考文档**：记录了接待设置的原始需求来源和字段设计思路，供架构设计参考
2. **字段 schema 参考**：其中的字段定义可作为数据库表结构设计和前端表单设计的输入
3. **不再作为运行时文件**：系统不再直接读取此文件，改为从数据库读取结构化数据，动态组装 prompt

### 11.5 技术方案落地要点

上述设计在**技术方案/架构设计阶段**需落地为：

1. **数据库表设计**：在 `user_profiles` 表中扩展接待设置相关字段，或新建 `ai_reception_settings` 表（推荐后者，职责分离）
2. **前端引导流程**：分步表单组件，含示例值、校验规则、安全检测提示
3. **服务端 prompt 组装模块**：从数据库读取结构化字段 → 按模板组装为 prompt 片段 → 注入 system prompt
4. **安全扫描中间层**：自由文本字段的红线关键词检测（输入时提示 + 注入时过滤双重保障）
5. **预览 API**：接收接待设置数据，返回模拟对话结果，供前端预览页面使用
